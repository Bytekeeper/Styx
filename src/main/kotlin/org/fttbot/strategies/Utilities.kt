...
object Utilities {
    ...
    val workerUtilization by LazyOnFrame {
        min(min(1.0, (UnitQuery.myWorkers.size.toDouble() + UnitQuery.myEggs.count { it.buildType.isWorker }) / 1.6 / (0.1 + MyInfo.myBases.sumBy {
            UnitQuery.minerals.inRadius(it, 300).count() + UnitQuery.myBuildings.count { it is GasMiningFacility } * 3
        })), (240 - UnitQuery.myWorkers.size) / 240.0)
    }
...
//    }
    ...
    val moreLingsUtility by LazyOnFrame {
        clamp((FTTBot.self.minerals() - 3 * FTTBot.self.gas()) * 0.001, 0.0, 1.0)
    }
